...
source("region_utils.R")  
source("filtering_rules.R")  
...
process_file_in_chunks <- function(file, table_name, rule, chunk_size = 1e6) {
  # If you want to pre-select columns from the Parquet to reduce I/O,
  # build col_select from the columns you truly need:
  # e.g. parse them from rule$select plus the columns used in filter.
  #
  # For demonstration, we skip that step because your filter uses data.table syntax.
  # If you know exactly which columns are used, you could do:
  # required_cols <- unique(c("BIDTYPE","DIRECTION","DUID","SETTLEMENTDATE", ...))
  # arrow_table <- read_parquet(file, as_data_frame = FALSE, col_select = required_cols)
  #
  # If you want to keep it simple, read all columns:
