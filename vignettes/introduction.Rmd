...
-   **Download location**:
    ...
    `data/raw_data/<dataset_name>/<YYYYMMDD>/...`

-   **Extraction**:

    -   Every ZIP is unpacked into `data/raw_data/.../<YYYYMMDD>/`
...
-   **Parquet output**:
    ...
    `data/parquet_tables/<TABLE_NAME>/<YYYYMMDD>.parquet`

-   **Cleanup**: original CSVs are deleted to save space.

-   **Mapping file**: `mapping/dataset_table_mapping.rds`

## 2. Filter & Enrich

Once you have raw Parquet under `data/parquet_tables/`, apply your filtering rules:
...
-   Reads each raw parquet from

    `data/parquet_tables/<TABLE_NAME>/`
...
-    Writes **filtered** Parquet under

    `data/filtered_parquet_tables/<TABLE_NAME>/`
...
AemoETL ships with a Shiny app under **inst/app/**. To launch it:
